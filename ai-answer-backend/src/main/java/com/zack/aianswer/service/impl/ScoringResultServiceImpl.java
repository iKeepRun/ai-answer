...
import com.zack.aianswer.mapper.ScoringResultMapper;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        Integer isDelete = scoringResult.getIsDelete();
        // 创建数据时，参数不能为空
        if (add) {
            //补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR,"评分结果不能为空");
        }
        // 修改数据时，有参数则校验
        //补充校验规则
